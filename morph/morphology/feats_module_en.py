...
class Feats_module_en:
    def __init__(self):
        self.neg_set = {'no', 'not', 'nothing', 'never'}# еще тут должны быть слова на 'un-', 'im-', 'dis-'
    ...
    def filter_feats_en(self, token, lemma, pos, feats, semclass, semslot):
        ...
        #neg_check = 0
            ...
            #for f in self.abbr_set:
                #'''штуки для фичей аббр, нег, форин'''
                #if 'SpecialLexemes' in feats and feats['SpecialLexemes'][0] in self.abbr_set and token == lemma:
                    #abbr_check = 1 #отмечаем как аббревиатуру
                #pattern = re.compile()
                #if pattern.search(token):
                    #foreign_check = 1
            #if 'Negative' in feats.values() or lemma in self.neg_set:
                #neg_check = 1
            ...
            if pos in self.pos_feats:
                ...
                if pos == 'AUX': #модальные глаголы (GTVerbModal) - вспомогательные и финитные
                    ...
                    #опять говно с причастиями и герундиями разребем (надеюсь, что сработает как для лексических глаголов)
                ...
                #'PartletOfSpeech': ['NumeralCardinal'] 'PartletOfSpeech': ['NumeralOrdinal'] NUM 
